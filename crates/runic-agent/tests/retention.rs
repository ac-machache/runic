...
use harness::*;
use runic_agent::{Runner, SpilledArtifact, ToolOutputSpill};
...
fn persisted_payloads(agent: &Runner) -> Vec<ToolResultPayload> {
    agent
        .state()
...
async fn artifact_retention_spills_bytes_and_persists_the_reference() {
    ...
    let mut agent = Runner::builder(provider.clone(), "tenant-1", "session-1")
        .model("test")
...
async fn artifact_retention_without_a_store_degrades_the_same_way() {
    ...
    let mut agent = Runner::builder(provider, "t", "s")
        .model("test")
        .tool(Arc::new(SpillingTool {
