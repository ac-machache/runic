//! Core message and streaming event types for the runic harness.
//!
//! Vendored and trimmed from jcode-message-types — preserves the canonical
//! provider-facing shapes (Role, Message, ContentBlock, StreamEvent,
//! ToolDefinition, ToolCall) and the small set of helper functions that the
//! agent loop needs.
...
impl ToolDefinition {
    ...
    /// Approximate prompt-token cost using the standard chars/4 heuristic.
...
///
/// Tool IDs from one provider may contain characters another provider rejects.
...
/// Connection phase for observability.
...
mod tests {
    ...
    fn sanitize_tool_id_replaces_invalid_chars() {
        ...
        assert_eq!(sanitize_tool_id("ok-id_42"), "ok-id_42");
    }
}
