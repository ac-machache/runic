...
fn convert_response(resp: GeminiResponse) -> Result<CompletionResponse, ProviderError> {
    let candidate = resp
        .candidates
        .into_iter()
        .next()
        .ok_or_else(|| ProviderError::Parse("No candidates in Gemini response".to_string()))?;
    ...
    async fn complete(
        ...
        for attempt in 0..=max_retries {
            ...
            if !resp.status().is_success() {
                ...
                if status == 404 {
                    return Err(ProviderError::ModelNotFound(message));
                }
