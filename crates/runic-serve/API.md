...
An HTTP + SSE surface over a runic `Agent`. A **session** is a session; a **run**
...
All data (sessions, artifacts, asks) is scoped by tenant, so tenant A can never
...
| `not_found`      | 404    | unknown session or run                           |
...
curl -sX POST localhost:8080/sessions \
  -H 'x-runic-tenant: alice' -H 'content-type: application/json' \
  -d '{"session_id":"t1","label":"my chat"}'

curl -s localhost:8080/sessions/t1 -H 'x-runic-tenant: alice'          # fetch
curl -sX PATCH localhost:8080/sessions/t1 -H 'x-runic-tenant: alice' \
  -H 'content-type: application/json' -d '{"label":null}'             # clear label
curl -s 'localhost:8080/sessions?limit=50' -H 'x-runic-tenant: alice' # list (paged)
curl -sX DELETE localhost:8080/sessions/t1 -H 'x-runic-tenant: alice' # delete + artifacts
```

`GET /sessions` pages newest-active-first; when more remain the response carries
`next_cursor`, which you pass back as `?cursor=`.

## Run streaming

`POST /sessions/{id}/runs/stream` drives a turn and streams events as
`text/event-stream`. Body is either `{"message":"..."}` or a full
`{"content":[...]}` block array.

```bash
curl -N -X POST localhost:8080/sessions/t1/runs/stream \
  -H 'x-runic-tenant: alice' -H 'content-type: application/json' \
  -d '{"message":"hello"}'
```
...
```bash
curl -sX POST localhost:8080/sessions/t1/asks/<ask_id> \
  -H 'x-runic-tenant: alice' -H 'content-type: application/json' \
...
`GET /sessions/{id}/runs/{run_id}/stream` replays a run's persisted events, then
...
```bash
curl -N localhost:8080/sessions/t1/runs/<run_id>/stream \
  -H 'x-runic-tenant: alice' -H 'last-event-id: 12'
```

`GET /sessions/{id}/events` returns the same log as a paged JSON snapshot
...
never enters a session). Requires an `audio/*` `Content-Type`; returns
