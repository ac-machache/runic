...
proptest! {
    ...
    fn decode_entities_never_panics(s in any::<String>()) {
        let _ = runic::builtin::decode_entities(&s);
    }

    #[test]
    fn html_to_text_never_panics(s in any::<String>()) {
        let _ = runic::builtin::html_to_text(&s);
    }

    #[test]
    fn eval_never_panics(s in any::<String>()) {
        let _ = runic::builtin::eval_calc(&s);
    }
    ...
    fn decode_entities_noop_without_amp(s in "[^&]{0,120}") {
        prop_assert_eq!(runic::builtin::decode_entities(&s), s);
    }
    ...
    fn eval_matches_reference(e in expr()) {
        let got = runic::builtin::eval_calc(&e.render()).expect("generated expr is valid");
