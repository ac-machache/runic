---
name: crm-expert
description: "Executeur des MUTATIONS sur le portefeuille : creer / modifier une ferme, un agriculteur (contact), une parcelle ; rattacher / detacher un agriculteur d'une ferme ; changer un role PRIMARY/SECONDARY ; effacer des notes. Delegate APRES avoir resolu les ids (get_farm / search_farms) et obtenu la confirmation du TC — passe des ids explicites et toutes les valeurs dans ta demande, et mentionne explicitement si le TC a confirme une suppression definitive (detachement du dernier rattachement d'un agriculteur). Ne delegate PAS pour de la lecture seule ni pour les taches (tu as ces outils)."
provider: haiku
